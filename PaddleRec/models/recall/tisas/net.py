...
class TiSASRecLayer(paddle.nn.Layer):
    def __init__(self, user_num, item_num, hidden_units, maxlen, time_span, num_blocks, num_heads, dropout_rate=0.2):
        super().__init__()
        ...
        self.item_emb = paddle.nn.Embedding(self.item_num + 1, hidden_units, padding_idx=0,
                                            weight_attr=paddle.ParamAttr(
                                                initializer=paddle.nn.initializer.XavierUniform()))
        self.item_emb_dropout = paddle.nn.Dropout(p=dropout_rate)

        self.abs_pos_K_emb = paddle.nn.Embedding(maxlen, hidden_units,
                                                 weight_attr=paddle.ParamAttr(
                                                     initializer=paddle.nn.initializer.XavierUniform()))
        self.abs_pos_V_emb = paddle.nn.Embedding(maxlen, hidden_units,
                                                 weight_attr=paddle.ParamAttr(
                                                     initializer=paddle.nn.initializer.XavierUniform()))
        self.time_matrix_K_emb = paddle.nn.Embedding(time_span + 1, hidden_units,
                                                     weight_attr=paddle.ParamAttr(
                                                         initializer=paddle.nn.initializer.XavierUniform()))
        self.time_matrix_V_emb = paddle.nn.Embedding(time_span + 1, hidden_units,
                                                     weight_attr=paddle.ParamAttr(
                                                         initializer=paddle.nn.initializer.XavierUniform()))
        ...
        for _ in range(num_blocks):
            new_attn_layernorm = paddle.nn.LayerNorm(hidden_units, epsilon=1e-8)
    ...
    def seq2feats(self, log_seqs, time_matrices):
        ...
        positions = paddle.arange(log_seqs.shape[1]).unsqueeze(0).expand([log_seqs.shape[0], -1])
        ...
        seqs *= (log_seqs != 0).astype(paddle.get_default_dtype()).unsqueeze(-1)  # broadcast in last dim
    ...
    def forward(self, log_seqs, time_matrices, item_indices=None, pos_seqs=None, neg_seqs=None):  # for training
